...
public class MoaSGDModelSerializer implements Serializer<MoaSGDModel> {

    @Override
    public byte[] serialize(String topic, MoaSGDModel data) {
        if (data == null)
            return null;
        try (ByteArrayOutputStream byteStream = new ByteArrayOutputStream();
          ...
          //  for ( int i=0 ; i< data.getAlgorithmParameters().size(); i++)
            ...
            try (ObjectOutputStream objectOut = new ObjectOutputStream(dataOut)) {
                objectOut.writeObject(data.getLearner());
